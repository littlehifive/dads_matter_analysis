...
# load data
dat_s <- read_csv(here::here("data/dads_matter_fa.csv"))
```
...
#zDysphoria
dat_s <- dat_s %>% 
  mutate(zDysphoria = ifelse(cidi1.1_2 == 1 & cidi2.1_2 < 4 & 
...
#zAnhedonia
dat_s <- dat_s %>% 
  mutate(zAnhedonia = ifelse(AnhedoniaScale > 2, 1, 0))

#zMajorDep
dat_s <- dat_s %>% 
  mutate(zMajorDep = ifelse(zDysphoria == 1 | zAnhedonia == 1, 1, 0))
...
cts_compromise_self =~ cts1 + cts7
cts_compromise_partner =~ cts2 + cts8

# verbal abuse (and also destroy other's belonging)
cts_verbal_abuse_self =~ cts3 + cts13
cts_verbal_abuse_partner =~ cts4 + cts14

# physical injury as a result of an argument/fight
cts_injury_self =~ cts5 + cts15
cts_injury_partner =~ cts6 + cts16

# physical abuse
cts_phyiscal_abuse_self =~ cts9 + cts11
cts_phyiscal_abuse_partner =~ cts10 + cts12

# sexual abuse
cts_sex_abuse_self =~ cts17 + cts19
cts_sex_abuse_partner =~ cts18 + cts20
"
```
